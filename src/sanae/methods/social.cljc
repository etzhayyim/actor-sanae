(ns sanae.methods.social
  "social.cljc — sanae DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from sanae's own
...
(defn draft-observation-post
  "A dry-run observation post over sanae's own domain (Field-agriculture robotics — sow/weed/harvest automation for the single largest pool of human toil on Earth (ISIC A01 / ISCO 6,92 / UNSPSC…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "sanae R0: live social posting is Council Lv6+ + operator + "
